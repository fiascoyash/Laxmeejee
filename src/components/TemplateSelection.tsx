...
const CATEGORY_COLORS: Record<TemplateCategory, string> = {
  professional: 'bg-slate-100 text-gray-700',
  gst: 'bg-green-100 text-green-700',
  retail: 'bg-orange-100 text-orange-700',
  modern: 'bg-slate-100 text-emerald-700',
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-slate-800 mb-2">Select Quotation Template</h2>
        <p className="text-slate-500">Choose a template design for your quotation</p>
      </div>
      ...
      {templates.length === 0 && (
        <div className="text-center py-12 bg-slate-50 rounded-lg border-2 border-dashed border-slate-300">
          <Layout className="w-12 h-12 mx-auto text-slate-400 mb-4" />
          <p className="text-slate-600 mb-4">No templates available</p>
          <button
  ...
  return (
    <button
      ...
      className={`relative bg-white rounded-xl border-2 p-4 text-left transition-all hover:shadow-lg ${
        isSelected ? 'border-emerald-500 ring-2 ring-emerald-200' : template.isPremium ? 'border-amber-300 hover:border-amber-400' : 'border-slate-200 hover:border-slate-300'
      }`}
    >
      {/* Category badge */}
      {template.category && (
